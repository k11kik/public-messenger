"""
plot
* pytplot (original)を用いたplot tool
...
def _normal(
    ...
    if xlim is None:
        ...
        ide = np.abs(dat.times - time_double(xlim[1])).argmin()
...
def _overplot(
    ...
    for var_plot in vars_overplot:
        ...
        mdat = get_data(var_plot, metadata=True)
    ...
    if not vars_overplot_spec:  # no spec data
        ...
        for i, var in enumerate(vars_overplot_nonspec):
            ...
            mdat_var = get_data(var, metadata=True)
        ...
        mdat_overplot_spec = get_data(var_overplot_spec, metadata=True)
...
def tplot_with_orbit_labels(
    ...
    for i, var in enumerate(vars_plot):
        ...
        if isinstance(var, str):
            dat = get_data(var)
            mdat = get_data(var, metadata=True)
            extras = mdat.get('plot_options', {}).get('extras', {})
